...
public class AdminDiscountManagerImplImpl extends DiscountManagerImplImpl implements AdminDiscountManagerImpl {
	 ...
	 * @generated NOT
	 */
	public void setAmountLimit(Discount discount, int amount) {
		if(discount != null) {
			DiscountLimit limit = discount.getDiscountLimit();
			if(limit != null && amount > 0) {
				limit.setTimesLeftToUse(amount);
				discountDataService.set(discount);
			}
		}
	}
	 ...
	 * @generated NOT
	 */
	public void addAllowedUsers(Discount discount, EList<LegalEntity> allowedUsers) {
		if(discount != null && allowedUsers != null && allowedUsers.size() > 0) {
			DiscountLimit limit = discount.getDiscountLimit();
			if(limit != null) {
				limit.getAllowedUsers().addAll(allowedUsers);
				discountDataService.set(discount);
			}
		}
	}
	 ...
	 * @generated NOT
	 */
	public void createDiscountLimitForDiscount(Discount discount, Date startDate, Date endDate, EList<LegalEntity> allowedUsers, int timesLeftToUse) {
		if(discount != null) {
			DiscountLimit limit = new DiscountLimitImpl(startDate, endDate, allowedUsers, timesLeftToUse);
